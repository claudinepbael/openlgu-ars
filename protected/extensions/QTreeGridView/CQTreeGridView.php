...
class CQTreeGridView extends CGridView
{
    ...
    public function init()
    {
        ...
        if(count($this->dataProvider->data)) {
            $left = $this->dataProvider->data[0]->category_acct_code;
            $right = $this->dataProvider->data[0]->parent_category_acct_code;
            $level = 1;//"parent_category_acct_code";
            $stack = array();
            $currentLevel = 0;
            $previousModel = null;
            try {
                foreach($this->dataProvider->data as $model) {
                    if($model->$level==1) { //root with level=1
                        $model->parent_category_acct_code = 0;
                        $currentLevel = 1;
                    } else {
                        if($model->$level == $currentLevel) {
                            ...
                            $model->parent_category_acct_code = $stack[count($stack)-1]->getPrimaryKey();
                        } elseif($model->$level > $currentLevel) {
                            if(is_null($previousModel)) {
                                throw new Exception('Tree is corrupted');
                            }
                            $currentLevel = $model->$level;
                            $model->parent_category_acct_code = $previousModel->getPrimaryKey();
    ...
    /**
     * Renders a table body row with id and parent_category_acct_code, needed for ActsAsTreeTable
     * jQuery extension.
    ...
    public function renderTableRow($row)
    {
        ...
        $parentClass = $model->parent_category_acct_code
                       ?'child-of-'.$model->parent_category_acct_code.' '
                       :'';
